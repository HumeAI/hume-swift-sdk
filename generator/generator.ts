...
type Direction = "sent" | "received" | "both";
...
const schemaToSwiftType = (
  ...
  switch (schema.kind) {
    ...
    case "unionOfRefs": {
      // Handle union of references (like tool_result which can be ToolResponseMessage | ToolErrorMessage | null)
      // For now, we'll generate a simple enum with associated values
      const unionName = swiftName(schema, surroundingPropertyName(back)!) || "UnionType";
      const cases: Array<{ caseName: string; type: SwiftType }> = [];
      
      if (schema.anyOf) {
        for (const variant of schema.anyOf) {
          if (variant.kind === "ref") {
            const referencedSchema = lookupSchemaRef(variant.$ref, allSchemas);
            const typeName = swiftName(referencedSchema);
            const caseName = camelCase(typeName);
            cases.push({ 
              caseName, 
              type: { type: "Reference", name: typeName } 
            });
          }
        }
      }
      ...
      if (cases.length === 0) {
        return todo("unionOfRefs with no valid cases");
      }
    ...
    case "object": {
      ...
      const rawProperties: Array<{ name: string; type: SwiftType }> =
        Object.entries(schema.properties)
          .map(([propName, prop]): { name: string; type: SwiftType } | null => {
            ...
            // unionOfRefs is now supported, no need to skip
            ...
            // Check if this property is required (using original property name)
            const isRequired = schema.required.includes(propName);
            
            // Only wrap in Optional if it's not required AND not already optional
            // The recursion above already wraps nullable types in Optional
            let finalType = type;
            if (!isRequired && type.type !== "Optional") {
              finalType = { type: "Optional" as const, wrapped: type };
            }
            
            return { name: camelCase(propName), type: finalType };
          })
          .filter(outNulls);
...
const openapiOperationToSDKMethod = (
  ...
  sdkParameters.push(
    ...specParameters
      ...
      .map((p): SDKMethodParam => {
        const { expr } = schemaToSwiftType(p.schema, allSchemas, null, "sent");
        let type = expr;
  ...
  if (op.kind === "jsonBody") {
    ...
    const bodyStructName = swiftName(resolveIfRef(bodySchema, allSchemas));
    sdkParameters.push({
      name: "request",
  ...
  if (isBinaryResponse("200") || isBinaryResponse("201")) {
    ...
    const { expr } = successResponseSchema
      ? schemaToSwiftType(successResponseSchema, allSchemas, null, "received")
...
const renderSwiftEnum = (def: SwiftEnum): string => {
  // Determine protocol conformance based on direction
  let protocolConformance = "Codable";
  if (def.direction === "sent") {
    protocolConformance = "Encodable";
  } else if (def.direction === "received") {
    protocolConformance = "Decodable";
  }
  
  return `
    public enum ${def.name}: String, ${protocolConformance} {
...
const renderSwiftDiscriminatedUnion = (
  ...
  // Only include decoder for types that can be received
  const shouldIncludeDecoder = def.direction !== "sent";
  // Only include encoder for types that can be sent
  const shouldIncludeEncoder = def.direction !== "received";

  const decoderCode = shouldIncludeDecoder ? `
  ...
  const encoderCode = shouldIncludeEncoder ? `
  ...
  return `
public enum ${def.name}: ${protocolConformance} {
    ${cases}
...
const renderSwiftStruct = (struct: SwiftStruct) => {
  ...
  // Determine protocol conformance based on direction
  let protocolConformance = "Codable";
  if (struct.direction === "sent") {
    protocolConformance = "Encodable";
  } else if (struct.direction === "received") {
    protocolConformance = "Decodable";
  }
  ...
  const initParameters = filteredProperties
    .map((prop) => `${prop.name}: ${renderSwiftType(prop.type)}`)
    .join(", ");
  
  const initAssignments = filteredProperties
...
${initAssignments}
  ...
  return `public struct ${struct.name}: ${protocolConformance} {
    ${filteredProperties
...
const renderSwiftDefinition = (
  ...
  const path = `${basePath}/Sources/Hume/API/${namespaceName}/Models/${def.name}.swift`;
  if (def.type === "enum") {
    return {
      path,
...
const renderNamespaceClient = (
  ...
  return {
    path: `${basePath}/Sources/Hume/API/${namespaceName}/Client/${namespaceName}Client.swift`,
    content: `
    import Foundation
...
const renderResourceClient = (
  ...
  return {
    path: `${basePath}/Sources/Hume/API/${namespaceName}/Resources/${resourceName}/${resourceName}Client.swift`,
    content: `
    import Foundation
...
const calculateSchemaDirections = (
  allEndpoints: Array<Endpoint>,
  allChannels: OA.AsyncAPISpec["channels"],
): Map<string, Direction> => {
  const ret: Map<string, Direction> = new Map();
  const setDirection = (schema: OA.JsonSchema, direction: Direction) => {
    if ("schemaKey" in schema && typeof schema.schemaKey === "string") {
      const existing = ret.get(schema.schemaKey);
      if (existing && existing !== direction) {
        ret.set(schema.schemaKey, "both");
      } else {
        ret.set(schema.schemaKey, direction);
      }
    } else if (schema.kind === "ref") {
      // For refs, extract the schema name and set direction directly
      const refName = schema.$ref.replace(/^#\/components\/schemas\//, "");
      const existing = ret.get(refName);
      if (existing && existing !== direction) {
        ret.set(refName, "both");
      } else {
        ret.set(refName, direction);
      }
    }
  };

  // Process AsyncAPI channels
  for (const channelPath in allChannels) {
    const channel = allChannels[channelPath];
    // Publish messages are sent by client
    walkAsyncAPIMessage(channel.publish.message, (m) => {
      if (m.kind === "message") {
        walkSchema(m.payload, (s) => setDirection(s, "sent"));
      }
    });
    // Subscribe messages are received by client
    walkAsyncAPIMessage(channel.subscribe.message, (m) => {
      if (m.kind === "message") {
        walkSchema(m.payload, (s) => setDirection(s, "received"));
      }
    });
  }

  // Process OpenAPI endpoints
  ...
  // Manually set known direction patterns for key schemas
  const knownDirections: Record<string, Direction> = {
    // AsyncAPI messages - received from server
    "AssistantMessage": "received",
    "AssistantEnd": "received", 
    "UserMessage": "both", // Can be sent by client and received in responses
    "UserInterruption": "received",
    "ChatMetadata": "received",
    "ToolCallMessage": "received",
    "ToolResponseMessage": "sent",
    "ToolErrorMessage": "sent",
    "PauseAssistantMessage": "sent",
    "ResumeAssistantMessage": "sent",
    
    // Common types used in both directions
    "AudioConfiguration": "both",
    "SessionSettings": "both",
    "Context": "both",
    "Tool": "both",
    "Inference": "received",
    "EmotionScores": "received",
    "ProsodyInference": "received",
    
    // TTS types - typically sent in requests
    "PostedTts": "sent",
    "PostedVoice": "sent",
    "PostedUtterance": "sent",
    "PostedContextWithUtterances": "sent",
    "PostedContextWithGenerationId": "sent",
    "PostedUtteranceVoiceWithId": "sent",
    "PostedUtteranceVoiceWithName": "sent",
    
    // TTS response types
    "ReturnTts": "received",
    "ReturnVoice": "received",
    "ReturnGeneration": "received",
    "ReturnPagedVoices": "received",
    
    // Error types - typically received
    "ErrorResponse": "received",
    "HttpValidationError": "received",
    "ValidationError": "received",
    "WebSocketError": "received",
  };

  // Apply known directions
  for (const [schemaName, direction] of Object.entries(knownDirections)) {
    ret.set(schemaName, direction);
  }
...
const buildSwiftSdk = (specs: OA.KnownSpecs): SwiftSDK => {
  ...
  Object.entries(allSchemas).forEach(([name, schema]) => {
    ...
    const { expr, defs } = schemaToSwiftType(schema, allSchemas, null, direction);
    if (expr.type === "TODO") {
...
const writeSwiftSdk = async (sdk: SwiftSDK, basePath: string) => {
  ...
  await clearGeneratedFiles(basePath);
  ...
  const writeFile = async (file: File) => {
    ...
    await fs.mkdir(file.path.split("/").slice(0, -1).join("/"), {
      recursive: true,
    });
  ...
  for (const namespaceName in sdk.namespaces) {
    const namespace = sdk.namespaces[namespaceName];
    await writeFile(renderNamespaceClient(namespaceName, namespace.resourceClients.map(rc => rc.name), basePath));
    for (const resourceClient of namespace.resourceClients) {
      await writeFile(renderResourceClient(namespaceName, resourceClient.name, resourceClient.methods, basePath));
    }
    for (const definition of namespace.definitions) {
      await writeFile(renderSwiftDefinition(namespaceName, definition, basePath));
    }
  }
}
...
Examples:
  ...
  await writeSwiftSdk(renamedSdk, basePath);
}
