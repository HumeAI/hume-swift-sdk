// This file is generated by generator.ts
public struct PostedTts: Codable, Hashable {
  public let context: PostedTtsContext?
  public let format: Format?
  public let includeTimestampTypes: [TimestampType]?
  public let instantMode: Bool?
  public let numGenerations: Int?
  public let splitUtterances: Bool?
  public let stripHeaders: Bool?
  public let utterances: [PostedUtterance]
  public let version: OctaveVersion?

  public init(
    context: PostedTtsContext?,
    format: Format?,
    numGenerations: Int?,
    splitUtterances: Bool?,
    stripHeaders: Bool?,
    utterances: [PostedUtterance],
    instantMode: Bool?,
    includeTimestampTypes: [TimestampType]?,
    version: OctaveVersion?
  ) {
    self.context = context
    self.format = format
    self.numGenerations = numGenerations
    self.splitUtterances = splitUtterances
    self.stripHeaders = stripHeaders
    self.utterances = utterances
    self.instantMode = instantMode
    self.includeTimestampTypes = includeTimestampTypes
    self.version = version
  }
}
